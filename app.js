...
mongodb.MongoClient.connect(mongoURI)
    .then((client) => {
        ...
        db = client.db("student_management"); // Assign database reference to 'db' variable
    })
    .catch((err) => {
...
app.get("/students", (req, res) => {
    db.collection("students").find().toArray()
        ...
        .catch(err => res.status(500).json({ error: "An eroor ocurred while inserting students", details: err }));
});
...
app.get("/students/:id/edit", (req, res) => {
    ...
    db.collection("students").findOne({_id: objectID })
        .then(student => {
            if(!student){
               return res.status(404).json({ error: "student not found", details: err });
            }
